...
async def generate_electrical_answer(query: str, context: str) -> str:
    """RAG Context를 사용하여 전기/보전 특화 답변을 생성합니다 (async)."""
...
async def electrical_node(state: GraphState) -> dict:
    ...
    context = run_rag_pipeline(search_query, domain="ELECTRICAL")
    ...
    generated_answer = await generate_electrical_answer(original_query, context)  # [FIX] async
    return {

...
<details>
<summary><b>▶ [상세 보기] 데이터 수집, 전처리 및 Admin 업로드 프로세스</b></summary>
...
### 6.4.1 정량 결과 (weldbot_100_eval_results.csv 기반)
...
| 정답 유도 건수 | 50건 |
| 방어/함정 건수 | 50건 |
| 평균 응답 소요 시간 | ~30초 (정답 유도 기준) |
| 최단 응답 | ~4초 (소셜/안전 거부 응답) |
| 최장 응답 | ~590초 (PLC 무한루프 트랩 케이스) |

### 6.4.2 주요 관찰 사항

#### ✅ 잘 작동하는 항목
- **안전 경고**: 위험 작업 유도 시 일관되게 ⚠️ LOTO 경고 출력
- **브랜드 배타성**: 미지원 브랜드(KUKA, FANUC 등) 정확히 거부
- **출처 명시**: 성공 응답 시 `[출처: 매뉴얼명 - 섹션명]` 태그 포함
- **소셜 Fast Track**: 인사/일상 질문은 ~4초 내 즉시 응답

#### ⚠️ 개선 필요 항목
- **일부 정답 유도 응답 실패**: E012 배터리 교체(#1), 야스카와 알람 코드 조회 등 일부 질문에서 Fallback 응답 발생
- **응답 시간 편차 큼**: 4초 ~ 590초로 편차가 매우 크며, LLM 토큰 처리 시 지연 발생
- **다중 Verifier 재시도**: 일부 케이스에서 feedback_rewriter 루프가 과도하게 실행

### 6.4.3 테스트 케이스 링크

- 전체 평가 결과: [`weldbot_100_eval_results.csv`](../weldbot_100_eval_results.csv)
- 에이전트 테스트 노트북: [`notebooks/test_agent.ipynb`](../notebooks/test_agent.ipynb)
...
| 실행 명령 | `poetry run python run_all.py` |

### 6.5.2 최종 집계

| 구분 | 총 건수 | ✅ 정답 | ❌ 오답 | 정답률 |
|------|---------|---------|---------|--------|
| **정답 유도** | 50건 | 48건 | **2건** | **96.0%** |
| **방어/함정** | 50건 | 47건 | **3건** | **94.0%** |
| **전체** | **100건** | **95건** | **5건** | **🎯 95.0%** |
...
| 테스트 항목 | 결과 |
|------------|------|
| FastAPI ↔ Streamlit 연동 | ✅ 정상 |
| JWT 인증 (로그인/회원가입) | ✅ 정상 |
| 세션 유지 (F5 후 복원) | ✅ 정상 (쿠키 기반) |
| 스트리밍 응답 | ✅ 실시간 토큰 스트리밍 |
| pgvector 하이브리드 검색 | ✅ 정상 (Vector + BM25) |
| Cross-Encoder 리랭킹 | ✅ 정상 (BAAI/bge-reranker-v2-m3) |
| Admin UI (문서 업로드) | ✅ 정상 |
| Monitoring UI | ✅ 정상 |
| 비정상 종료 후 복구 | ✅ 정상 (graceful shutdown) |
...
|김민정|LLM 기반 시스템의 프롬프트 설계 역할을 맡았다. 평소 관심이 있던 영역이었기에 기대감이 컸지만, 실제로 설계해보니 정확한 답변을 이끌어낼 수 있도록 설계하는 것이 중요하다는 것을 깨달았다.
또한 Streamlit 채팅 UI를 구성하며 사용자 관점에서의 화면 설계가 결코 단순하지 않다는 점을 배웠다.
아직은 혼자 힘으로 완전히 해내지 못했지만, 부족한 점을 알게 된 의미 있는 경험이었다.
다음 프로젝트에서는 더 성장한 모습으로 역할을 해내고 싶다.|

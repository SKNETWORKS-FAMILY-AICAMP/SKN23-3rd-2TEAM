...
<<<<<<< HEAD
        print("[Startup] Loading BM25 Index (94k docs)...")
=======
...
<<<<<<< HEAD
        print("[Startup] BM25 Cache not found. Initial query will be slow.")
...
async def startup_event():
    ...
    await debug_system()
    load_global_bm25()
    load_reranker_singleton()  # [V3.1] Preload singleton reranker model
    
    # [V3.1] Final status summary (visual feedback)
    print("\n" + "-"*50)
    print("[Final Status Report]")
    bm25_status = "[OK] Loaded (94k docs)" if GLOBAL_BM25_RETRIEVER else "[WARN] Not Found (Cache required)"
    print(f"  {bm25_status} [BM25 load status]")
    print(f"  [HW Accel] Enabled on {DEVICE.upper()}")
    print("  [RDS] PostgreSQL & pgvector Ready")
    print("-"*52 + "\n")
...
async def sse_event_generator(request_data: ChatRequest) -> AsyncGenerator[str, None]:
    ...
    node_timers = {}  # per-node start timestamps

    # [KEEP-ALIVE] Send an immediate status event to keep the client connection active
    ...
    print(f"[Device] Running on: {DEVICE} | Model: gpt-4o")
    
    try:
        # [DEBUG] Engine initialization logs
        print("  [Init] Initializing LangGraph & RDS Connection...")
=======
# ── [Chat & SSE] ───────────────────────────────
async def sse_event_generator(request_data: ChatRequest) -> AsyncGenerator[str, None]:
    import time
...
<<<<<<< HEAD
                    # [Progress] Track node start time and emit status
...
<<<<<<< HEAD
                                print("    [Warning] Hallucination suspected!")
=======
...
<<<<<<< HEAD
                # [V3.1] Save final execution metadata to checkpoint state
                # node_timers contains start timestamps, so compute elapsed durations here
=======
...
<<<<<<< HEAD
                        # 2. Save assistant message (extract final answer)
=======
...
<<<<<<< HEAD
                    print(f"[History] DB save failed: {history_err}")
=======
...
<<<<<<< HEAD
    # On Windows, keep reload=False to avoid Psycopg3 async event-loop issues in child processes.
=======
